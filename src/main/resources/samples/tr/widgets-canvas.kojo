...
val açıklama = // html hyper-text-markup-language demek. İnternet sitelerinin dilidir!
    ...
    </html>.toString // toString Yazıya çevirir
  ...
  Resim.arayüz(
    ay.Sütun(
      ...
      ay.Sıra(ay.Tanıt("Hız: "), ay_hız, ay.Düğme("Üret ve canlandır") {
        val hız = ay_hız.value // saniyede bu kadar nokta ilerlesin
        val solKenarX = tuvalAlanı.x
        val resim = götür(solKenarX, 0) * boyaRengi(renkler(ay_renkler.value)) ->
        Resim {
          yinele(ay_kaçKenar.value) { // value kullanıcının girdiği değeri verir
            ileri(ay_uzunluk.value)
            sağ(ay_açı.value)
          }
        }
        çiz(resim)
