...
//  case class Point(x: Float, y: Float)
  ...
  trait Coords {
    ...
    def adjust(p: Point): Point = p match {
      case Point(x, y) =>
  ...
  sealed abstract class SVGCmd { def apply(): Unit }
  ...
  trait MoveTo extends Coords {
    ...
    def apply() = {
      if (pts.nonEmpty) adjust(pts.head) match {
        case Point(x, y) =>
  ...
  case class MoveToAbs(pts: List[Point]) extends SVGCmd with MoveTo with AbsoluteCoords

  case class MoveToRel(pts: List[Point]) extends SVGCmd with MoveTo with RelativeCoords
  ...
  case class LineToAbs(pts: List[Point]) extends SVGCmd with LineTo with AbsoluteCoords

  case class LineToRel(pts: List[Point]) extends SVGCmd with LineTo with RelativeCoords
  ...
  case class HLineToAbs(xs: List[Float]) extends SVGCmd with HorizontalLine with AbsoluteCoords

  case class HLineToRel(xs: List[Float]) extends SVGCmd with HorizontalLine with RelativeCoords
  ...
  case class VLineToAbs(ys: List[Float]) extends SVGCmd with VerticalLine with AbsoluteCoords

  case class VLineToRel(ys: List[Float]) extends SVGCmd with VerticalLine with RelativeCoords
  ...
  case class CurveToAbs(cs: List[CurvePointGroup]) extends SVGCmd with Curve with AbsoluteCoords

  case class CurveToRel(cs: List[CurvePointGroup]) extends SVGCmd with Curve with RelativeCoords
  ...
  case class SmoothCurveToAbs(cs: List[(Point, Point)]) extends SVGCmd with SmoothCurve with AbsoluteCoords

  case class SmoothCurveToRel(cs: List[(Point, Point)]) extends SVGCmd with SmoothCurve with RelativeCoords
  ...
  case class QuadBezierAbs(cs: List[QuadPointGroup]) extends SVGCmd with QuadBezier with AbsoluteCoords
  case class QuadBezierRel(cs: List[QuadPointGroup]) extends SVGCmd with QuadBezier with RelativeCoords
  ...
  case class SmoothQuadBezierAbs(cs: List[Point]) extends SVGCmd with SmoothQuadBezier with AbsoluteCoords

  case class SmoothQuadBezierRel(cs: List[Point]) extends SVGCmd with SmoothQuadBezier with RelativeCoords

  case class EllipticalArcAbs(as: List[_]) extends SVGCmd with AbsoluteCoords {
    def apply() = {}
  }

  case class EllipticalArcRel(as: List[_]) extends SVGCmd with RelativeCoords {
    def apply() = {}
  }
  ...
  class SVGPathParser extends JavaTokenParsers {
    def drawto: Parser[Any] =
      lineto | closepath | hlineto | vlineto | curveto | sCurveto | qBezierto | sqBezierto | elliptArc
