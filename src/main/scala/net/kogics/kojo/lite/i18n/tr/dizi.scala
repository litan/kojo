...
trait SeqMethodsInTurkish {
  ...
  implicit class colSeqYöntemleri[T](d: DeğişkenDizi[T]) {
    ...
    def önü: DeğişkenDizi[T] = d.init
    def sonu: T = d.last
    def boyu: Sayı = d.length
    def boşMu: İkil = d.isEmpty
    def doluMu: İkil = d.nonEmpty
    def ele(deneme: T => İkil): DeğişkenDizi[T] = d.filter(deneme)
    def eleDeğilse(deneme: T => İkil): DeğişkenDizi[T] = d.filterNot(deneme)
    def işle[A](işlev: T => A): DeğişkenDizi[A] = d.map(işlev)
    def düzİşle[A](işlev: T => DeğişkenDizi[A]): DeğişkenDizi[A] = d.flatMap(işlev)
    def sıralı(implicit ord: Ordering[T]): DeğişkenDizi[T] = d.sorted(ord)
    def sırala[A](i: T => A)(implicit ord: Ordering[A]): DeğişkenDizi[T] = d.sortBy(i)
    def sırayaSok(önce: (T, T) => İkil): DeğişkenDizi[T] = d.sortWith(önce)
    ...
    def yinelemesizİşlevle[T2](işlev: T => T2): DeğişkenDizi[T] = d.distinctBy(işlev)
    ...
    def tersi = d.reverse
    def değiştir[S >: T](yeri: Sayı, değeri: S): DeğişkenDizi[S] = d.updated(yeri, değeri)
    ...
    def düşür(n: Sayı): DeğişkenDizi[T] = d.drop(n)
    def düşürDoğruKaldıkça(deneme: T => İkil): DeğişkenDizi[T] = d.dropWhile(deneme)
    def düşürSağdan(n: Sayı): DeğişkenDizi[T] = d.dropRight(n)
