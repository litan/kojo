...
class PictureCollisionTest extends FunSuite with Matchers with xscala.RepeatCommands {
  ...
  test("box with many boxes collision") {
    ...
    val p2 = fill(blue) * trans(psize, 0) -> testBox
    val p3 = fill(blue) * trans(2 * psize, 0) -> testBox
    val p4 = fill(blue) * trans(3 * psize / 2, psize / 2) -> testBox
  ...
  test("box with many boxes - non collision") {
    ...
    val p2 = fill(blue) * trans(psize, 0) -> testBox
    val p3 = fill(blue) * trans(2 * psize, 0) -> testBox
    val p4 = fill(blue) * trans(3 * psize / 2, psize + delta) -> testBox
  ...
  test("hvpics-hvpics non collision") {
    val p1 = trans(-psize / 2 - 1, 0) -> testHVPic
    val p2 = trans(psize / 2 + 1, 0) -> testHVPic
    p1.draw()
    p2.draw()
    p1.collidesWith(p2) should be(false)
  }
  ...
  test("hvpics-hvpics collision") {
    val p1 = trans(-psize / 2, 0) -> testHVPic
    val p2 = trans(psize / 2, 0) -> testHVPic
    p1.draw()
    p2.draw()
    p1.collidesWith(p2) should be(true)
  }
